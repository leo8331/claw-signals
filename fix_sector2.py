...
# Find L2 table section
l2_start = content.find('id="tbl-l2"')
if l2_start == -1:
    print("Could not find tbl-l2")
    exit(1)

# Find where L2 ends (start of L3 table)
l3_start = content.find('id="tbl-l3"')
if l3_start == -1:
    print("Could not find tbl-l3")
    exit(1)

# Extract L2 tbody content
l2_tbody_start = content.find('<tbody>', l2_start)
if l2_tbody_start == -1:
    print("Could not find tbody for L2")
    exit(1)

# L2 tbody ends right before </table> that ends L2 table
# We need to find the </tbody> tag before l3_start
l2_tbody_end = content.rfind('</tbody>', l2_tbody_start, l3_start)
if l2_tbody_end == -1:
    print("Could not find end of tbody for L2")
    # Try alternative: find </table> for L2
    l2_table_end = content.rfind('</table>', l2_tbody_start, l3_start)
    if l2_table_end == -1:
        print("Could not find end of table for L2")
        exit(1)
    # Work backwards to find the end of tbody
    # Assume tbody ends right before </table>
    l2_tbody_end = l2_table_end
    l2_rows = content[l2_tbody_start+7:l2_table_end].strip()
    # Remove </tbody> if present
    if '</tbody>' in l2_rows:
        l2_rows = l2_rows[:l2_rows.rfind('</tbody>')].strip()
else:
    l2_rows = content[l2_tbody_start+7:l2_tbody_end].strip()

print(f"L2 rows length: {len(l2_rows)}")

# Extract L3 tbody content
l3_tbody_start = content.find('<tbody>', l3_start)
if l3_tbody_start == -1:
    print("Could not find tbody for L3")
    exit(1)

# L3 table ends at </table> before <div class="ft">
ft_start = content.find('class="ft"', l3_start)
if ft_start == -1:
    print("Could not find footer")
    exit(1)

l3_table_end = content.rfind('</table>', l3_tbody_start, ft_start)
if l3_table_end == -1:
    print("Could not find end of table for L3")
    exit(1)

l3_tbody_end = content.rfind('</tbody>', l3_tbody_start, l3_table_end)
if l3_tbody_end == -1:
    l3_rows = content[l3_tbody_start+7:l3_table_end].strip()
else:
    l3_rows = content[l3_tbody_start+7:l3_tbody_end].strip()

print(f"L3 rows length: {len(l3_rows)}")
...
# Write output - directly overwrite the original
