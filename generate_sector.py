...
# Extract L2 and L3 table rows
l2_match = re.search(r'<table id="tbl-l2">.*?<tbody>(.*?)</tbody>', content, re.DOTALL)
l3_match = re.search(r'<table id="tbl-l3">.*?<tbody>(.*?)</tbody>', content, re.DOTALL)

l2_rows = l2_match.group(1).strip() if l2_match else ''
l3_rows = l3_match.group(1).strip() if l3_match else ''
...
# Write output
with open(r'D:\tdx临时\历史数据\2026-06-05\sector_new.html', 'w', encoding='utf-8') as f:
    f.write(html)
